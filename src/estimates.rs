...
impl AliceAndBobEstimates {
    ...
    pub fn physical_qubits(&self) -> u64 {
        ...
        //
        // The boundary patch between the factories and the main compute block is sized
        // to the smaller of the two code distances. `boundary_correction` adds the qubits
        // the `copies * 5` term below misses by sizing it off the factory distance alone.
        ...
        let (copies, boundary_correction) = self.toffoli_factory_part().map_or((0, 0), |part| {
            ...
            (
                part.copies(),
                2 * factory_distance.saturating_sub(main_distance),
            )
        });
